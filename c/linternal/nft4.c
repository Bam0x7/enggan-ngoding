...
// Fungsi untuk membuat ekspresi immediate
struct nftnl_expr* build_expr_immediate(uint32_t dreg, uint32_t imm_value) {
    struct nftnl_expr *e = nftnl_expr_alloc("immediate");
    nftnl_expr_set_u32(e, NFTNL_EXPR_IMM_DREG, dreg);  // Tujuan register
    nftnl_expr_set_u32(e, NFTNL_EXPR_IMM_DATA, imm_value);  // Nilai langsung
    return e;
}
...
int main(void) {
    ...
    // Add expression lookup (set)
    struct nftnl_expr *lookup_expr = nftnl_expr_alloc("lookup");
    nftnl_expr_set_u32(lookup_expr, NFTNL_EXPR_LOOKUP_SREG, NFT_REG_1);
    nftnl_expr_set_str(lookup_expr, NFTNL_EXPR_LOOKUP_SET, "my_set");
    nftnl_rule_add_expr(r, lookup_expr);

    // Add another expression (immediate)
    struct nftnl_expr *imm_expr = build_expr_immediate(NFT_REG_2, 0x1234);  // Menetapkan nilai langsung 0x1234 ke register 2
    nftnl_rule_add_expr(r, imm_expr);
